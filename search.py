...
def graph_search(problem, fringe):
    ...
    generated_nodes = 0 
    visited = 0
    fringe.append(Node(problem.initial)) #Lista de nodos a explorar
    while fringe:
        node = fringe.pop()
        if problem.goal_test(node.state):#Si encuentras la meta
            visited +=len(closed.keys())
            print(f"Se han visitado {visited+1} nodos")
            print(f"Se han generado {generated_nodes+1} nodos")
            print(f"Coste total de la ruta: {node.path_cost}") 
            return node
        if node.state not in closed:#Comprobación de nodo explorado
            closed[node.state] = True 
            successors = node.expand(problem) #Expande el nodo actual para encontrar todos sus sucesores.
            generated_nodes += len(successors) 
            fringe.extend(successors) 
        else:
            visited +=1#Si ya ha sido explorado aumenta en 1 los nodos visitados
