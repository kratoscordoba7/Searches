...
class FIFOQueue(Queue):
    ...
    def __len__(self):
        return len(self.A) - self.start #Resta la longitud de la lista abierta menos los visitados de la lista. Se elimina los visitados y se deja los no visitados

    def extend(self, items):
        self.A.extend(items)

    def pop(self):
        e = self.A[self.start] #Nodo inicial
        self.start += 1 #Puntero
        if self.start > 5 and self.start > len(self.A) / 2:
            self.A = self.A[self.start:]
            self.start = 0
        return e #Nodo a extraer y va a ser visitado
...
class BranchAndBound(Queue):
    ...
    def pop(self):
        #Quitar elementos de la lista abierta
        return self.A.pop()
...
class Branch_and_boundWS(Queue):
    ...
    def pop(self):
        return self.A.pop()#Te quita el último elemento de la lista y lo devuelve
